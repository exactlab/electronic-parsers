...
from .metainfo.quantum_espresso import (
    ...
    x_qe_section_bands_diagonalization,
        ...
        'xc_terms': [
            {
                ...
                'exx_compute_weight': lambda exx: 0.542 if (abs(exx) > 0.01) else 1.0,
            },
            {
                'XC_functional_name': 'GGA_X_PW91',
                'XC_functional_weight': 0.167,
                'exx_compute_weight': lambda exx: 0.167 if (abs(exx) > 0.01) else 0.0,
            },
        ],
        'xc_terms_remove': [
            {
                'XC_functional_name': 'LDA_X',
                'exx_compute_weight': lambda exx: 0.709 if (abs(exx) > 0.01) else 1.0,
            }
        ],
        'xc_section_method': {
...
class QuantumEspressoOutParser(TextParser):
    ...
    def init_quantities(self):
        ...
        def str_to_dispersion(val_in):
            ...
            return {
                v[0].strip(): {'vdw_radius': float(v[1]), 'C_6': float(v[2])}
                for v in val
            }
        ...
        header_quantities = [
            ...
            Quantity('x_qe_ibrav', r'bravais\-lattice index\s*=\s*(\d+)', dtype=int),
            Quantity(
                'alat',
            ...
            Quantity(
                'mixing_scheme', r'number of iterations used\s*=\s*(\d+)\s*(.*)mixing'
            ),
            Quantity(
                'xc_functional',
            ...
            Quantity('x_qe_md_max_steps', r'\s*nstep\s*=\s*(\d+)', dtype=int),
            Quantity(
                'spin_orbit_mode',
            ...
            Quantity(
                ...
                sub_parser=TextParser(
                    quantities=[
                        ...
                        Quantity(
                            'warning', r'(Number of k-points >= 100: set verbosity)'
                        ),
                    ]
                ),
            ),
            ...
            Quantity(
                ...
                str_operation=str_to_occupations,
                convert=False,
            ),
            Quantity(
                'allocated_arrays',
        ...
        energy_quantities = [
            Quantity(key, pattern, dtype=float, unit='rydberg')
            for key, pattern in self.energy_quantities.items()
        ]
        ...
        calculation_quantities = [
            ...
            Quantity(
                'number_of_planewaves', r'\(\s*(\d+)\s*PWs\)', dftype=int, repeats=True
            ),
            Quantity(
                'band_energies',
            ...
            Quantity(
                'output_datafile', r'Writing output data file ([\w\.]+)', dtype=str
            ),
            Quantity(
                'negative_rho',
                r'negative rho \(up, down\):\s*([\d\.E\-\+]+)\s*([\d\.E\-\+]+)',
                dtype=float,
            ),
            Quantity(
                'total_time',
        ...
        self._quantities = [
            Quantity(
                ...
                sub_parser=QuantumEspressoRunParser(quantities=run_quantities),
                repeats=True,
            )
        ]
...
class QuantumEspressoParser(BeyondDFTWorkflowsParser):
    ...
    def parse_scc(self, run, calculation):
        ...
        if eigenvalues is not None:
            ...
            reciprocal_cell = (
                sec_run.system[-1].x_qe_reciprocal_cell.to('1/m').magnitude
            )
            k_points = np.dot(np.linalg.inv(reciprocal_cell), k_points.T).T

            try:
                eigenvalues = (
                    np.reshape(eigenvalues, (n_spin, len(k_points), n_eigs)) * ureg.eV
                )
                occupations = calculation.get('occupation_numbers')
                if occupations is not None:
                    occupations = np.reshape(
                        occupations, (n_spin, len(k_points), n_eigs)
                    )
                ...
                if number_of_planewaves is not None:
                    number_of_planewaves = np.reshape(
                        ...
                        (n_spin, len(number_of_planewaves) // n_spin),
                    )
                    sec_eigenvalues.x_qe_eigenvalues_number_of_planewaves = (
                        number_of_planewaves[0]
                    )
        ...
        def parse_diagonalization(source, target):
            diagonalization = {
                ...
                if key.startswith('diagonalization')
            }
            if not diagonalization:
                return

            if (
                source.get('number') is not None
                or source.get('ecutwfc') is not None
                or source.get('beta') is not None
            ):
                metainfo_ext = '_scf'
        ...
        for iteration in calculation.get('iteration', []):
            ...
            if magnetic_moments is not None:
                ...
                sec_scf_iteration.x_qe_iter_mpersite_idx = [
                    int(i) for i in magnetic_moments[0]
                ]
            ...
            if time is not None:
                ...
                sec_scf_iteration.time_calculation = (
                    sec_scf_iteration.time_physical - initial_time
                )
    ...
    def parse_system(self, run, calculation):
        ...
        if labels_positions is not None:
            labels_positions = (
                labels_positions.get('labels'),
                _convert('positions', labels_positions),
            )
        if labels_positions is None:
            return
        ...
        if reciprocal_cell is None and simulation_cell is not None:
            ...
            reciprocal_cell = np.array(
                [np.cross(cell[(i + 1) % 3], cell[(i + 2) % 3]) for i in range(3)],
                dtype=float,
            )
        ...
        if k_points is not None:
            k_points = (
                np.array(k_points, dtype=float) * 2 * np.pi / run.get_header('alat')
            )
        ...
        for grid_type in ['dense', 'smooth']:
            ...
            setattr(
                sec_system, 'x_qe_%s_FFT_grid' % grid_type, [int(n) for n in grid[1:]]
            )
    ...
    def parse_configurations(self, run):
        ...
        dos_files = [
            p for p in os.listdir(self.out_parser.maindir) if p.endswith('.dos')
        ]
    ...
    def parse_method(self, run):
        ...
        if array is not None:
            ...
            sec_method.x_qe_allocated_array_size = (
                (array[1] * ureg.mebibyte).to('bit').magnitude
            )
        ...
        if array is not None:
            ...
            sec_method.x_qe_temporary_array_size = (
                (array[1] * ureg.mebibyte).to('bit').magnitude
            )
        ...
        if spin_orbit_mode is not None:
            sec_method.x_qe_spin_noncollinear = (
                spin_orbit_mode[0].strip().lower() == 'noncollinear'
            )
        ...
        if martyna_tuckerman_parameters is not None:
            sec_method.x_qe_isolated_system_method_martyna_tuckerman_alpha = (
                martyna_tuckerman_parameters[0]
            )
            sec_method.x_qe_isolated_system_method_martyna_tuckerman_beta = (
                martyna_tuckerman_parameters[1]
            )
        ...
        if starting_charge_negative_spin is not None:
            sec_method.x_qe_starting_charge_negative_up = starting_charge_negative_spin[
                0
            ]
            sec_method.x_qe_starting_charge_negative_down = (
                starting_charge_negative_spin[1]
            )
        ...
        for report in run.get_header('pseudopotential_report', []):
            ...
            pseudopotential_report[species] = {
                'version': report.get('version', '').strip(),
                'contents': report.get('contents'),
            }

        renormalized_wavefunction = {
            r[0]: ' '.join(r[1:])
            for r in run.get_header('renormalized_wavefunction', [])
        }

        atom_radii = {r[1]: r[0] * ureg.bohr for r in run.get_header('atom_radii', [])}
        ...
        for pp in pseudopotential:
            ...
            if radius is not None:
                sec_method_atom_kind.x_qe_species_integration_radius = radius.to(
                    'm'
                ).magnitude
            ...
            for key, val in dispersion.items():
                setattr(
                    sec_method_atom_kind, 'x_qe_dispersion_correction_%s' % key, val
                )

            atom_sp = atom_species.get(
                pp.get('label'), [None] * len(atom_species_names)
            )
    ...
    def parse(self, filepath, archive, logger):
        ...
        for run in self.out_parser.get('run', []):
            ...
            if date_time is not None:
                date_time = datetime.strptime(
                    date_time.replace(': ', ':0').replace(' ', ''), '%H:%M:%S%d%b%Y'
                )
